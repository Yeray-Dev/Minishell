...
void detect_builtin(struct s_cmd *cmd);
int exec_builtin(struct s_shell *sh, struct s_cmd *cmd);

// ---------- BUILTINS ----------
int builtin_pwd(struct s_shell *sh, struct s_cmd *cmd);
int builtin_echo(struct s_shell *sh, struct s_cmd *cmd);
int builtin_env(struct s_shell *sh, struct s_cmd *cmd);
int builtin_export(struct s_shell *sh, struct s_cmd *cmd);
int builtin_unset(struct s_shell *sh, struct s_cmd *cmd);
void builtin_exit(struct s_shell *sh, struct s_cmd *cmd);
int builtin_cd(struct s_shell *sh, struct s_cmd *cmd);
...
// ---------- FUNCIONES EXPORT ----------
void export_one(struct s_shell *sh, char *arg, int *has_error);
...
char *get_var_value(const char *key, struct s_shell *sh);
char *get_home_cached(struct s_shell *sh);
int needs_free(char *arg);
void print_chdir_error(char *path, char *arg);

/* cd_path.c */
char *get_parent_from_pwd(struct s_shell *sh);
...
/* cd_target.c */
char *get_target_path(struct s_shell *sh, char *arg);
int change_directory(struct s_shell *sh, char *path, char *arg);
void update_environment(struct s_shell *sh, char *oldpwd, char *pwd);
